...
app.post('/getNewsFileName', (req, res, next) => {
  db.query(`select  p.*
  ,GROUP_CONCAT(m.file_name) AS news_image
  from content p
   LEFT JOIN media m ON (p.content_id = m.record_id) 
   where p.content_id !='' AND p.content_type='News'
